...
LinkedList.prototype.remove = function () {
  ...
  if(!this.head.next){// esto es que el next que sigue al head es null   //!let current = this.head;
    this.head = null
    return null
  }
  else{
    let current = this.head;
    while (current.next.next) {// mientras tengas un objeto va a ser true ahora cuando el next sea null va a ser false 
      current = current.next
    }// avanza hasta un next anterior para poder borrar el ultimo 
    current.next = null; 
    let valueCopy = current.next.value;
    this._length--;
    return valueCopy
  }
